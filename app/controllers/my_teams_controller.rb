class MyTeamsController < ApplicationController
	...
	def saveRequest
		...
		if @request.save
			request_from = Team.find(@request.request_from)
			message = "You have a new request from team "+request_from.team_name+" for "+request_from.sport_id+"."

			Request.send_message(Team.find(params[:req_to]).player.mobile, message)
	...
	def acceptRequest
		...
		if request.update_attributes(status: 1)
			request_to = Team.find(request.request_to)
			message = "Your Request to team "+request_to.team_name+" for "+request_to.sport_id+" has been Accepted."

			Request.send_message(Team.find(request.request_from).player.mobile, message)
